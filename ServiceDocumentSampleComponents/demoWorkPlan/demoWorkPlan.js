...
export default class DemoWorkPlan extends LightningElement {
    ...
    handleWorkStepGql(val) {
        if (val && val.errors && val.errors.length > 0) {
            // ERROR
        } else if (val && val.data && val.data.uiapi.query.WorkStep && val.data.uiapi.query.WorkStep.edges.length > 0) {
            this._workStepRecords = val.data.uiapi.query.WorkStep.edges;
        }
    }

    get workStepQuery() {
        return gql`query workstep ($parentId: ID="") {
            uiapi {
                query {
                    WorkStep (where: {WorkPlanId: { eq: $parentId }}) @category(name: "recordQuery") {
                        edges {
                            node {
                                Id,
                                WorkPlanId {
