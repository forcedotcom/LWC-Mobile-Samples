...
export default class MobileAppointmentBookingLanding extends LightningElement {
    ...
    switch (this.workerAssignmentOptions) {
      case "1":
        ...
        if (this.isExcluded) {
          ...
          this.currentAssignmentMethod =
            assignmentMethod.ASSIGN_TO_ANY_AVIALABLE;
        } else {
      ...
      default: {
        console.log(
          "Error in calcAssignmentMethod did not receive valid settings"
        );
        break;
      }
    }
  ...
  calculateEarliestStartPermittedAndDueDateForServiceAppointment(SAdata) {
    if (
      SAdata.EarliestStartTime &&
      new Date(SAdata.EarliestStartTime) > new Date()
    ) {
    ...
    if (this.schedulingHorizonValue && this.schedulingHorizonValue !== "null") {
      ...
      let schedulingHorizonValueToNumber = parseInt(
        this.schedulingHorizonValue,
        10
      ); // Lint fixes : added 10 as redix param;
      horizonDate = new Date(
        tempDate.setDate(tempDate.getDate() + schedulingHorizonValueToNumber)
      );
    ...
    getWorkTypeOffset({ workTypeName: SAdata.WorkTypeName })
      .then((data) => {
        let workTypeDueDateOffset =
          data > -1
            ? new Date(
                new Date(this.minValidCalendarDate).getTime() + data * 60 * 1000
              )
        ...
        if (data.error || data.DML) {
          ...
          scheduleSA({
              ...
              if (scheduleData.error || scheduleData.success === "null") {
                ...
                console.log(
                  "Check if the SA " +
                    ...
                    " can be scheduled via the Gantt"
                );
                this.template
                  .querySelector(
            ...
            .catch((error) => {
              this.revertSA();
              console.log(
                "Error while executing FSL API : " + JSON.stringify(error)
              );
              this.template
                .querySelector(
                  "c-mobile-appointment-booking-scheduling-container"
  ...
  setAssigNameByAssignMethod() {
    if (this._currentAssignmentMethod) {
      if (this._currentAssignmentMethod === assignmentMethod.ASSIGN_TO_ME) {
        this.assignToName =
          this.LABELS.FollowUpAppointments_assigned_to_you.replace(
            "{0}",
            this.userName
          );
      } else {
        this.assignToName =
          this.LABELS.FollowUpAppointments_assigned_to_any_available_worker;
      }
    }
  }
