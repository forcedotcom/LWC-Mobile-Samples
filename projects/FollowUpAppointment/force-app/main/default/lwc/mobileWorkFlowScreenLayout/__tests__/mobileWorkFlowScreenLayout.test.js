...
describe("c-mobile-work-flow-screen-layout", () => {
  ...
  it("should call left button when left arrow clicked", async () => {
    ...
    action.click();
    return Promise.resolve().then(() => {
      expect(element.handleLeftButtonEventClick).toHaveBeenCalledTimes(0);
    });
  });
  ...
  it("should call right button when right arrow clicked", async () => {
    ...
    return Promise.resolve().then(() => {
      //expect(element.handleRightButtonEventClick).toHaveBeenCalledTimes(1);
  ...
  it("should call edit button", async () => {
    ...
    action.click();
    return Promise.resolve().then(() => {
      expect(element.handleClildEditButtonEvent).toHaveBeenCalledTimes(0);
    });
  });
  ...
  it("Getter and setter for selectedfieldlist", () => {
    ...
    expect(element.selectedfieldlist).toStrictEqual(["title2"]);
    expect(spy).toHaveBeenCalled();
  });
