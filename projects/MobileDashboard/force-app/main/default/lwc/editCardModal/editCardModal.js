...
export default class EditCardModal extends LightningModal {
  ...
  onObjectComboChange = (e) => {
    this.tempCard = {
      ...
      object: {
        ...
        label: e.target.options.find((opt) => opt.value === e.detail?.value)
          ?.label
      },
      filter: {
  ...
  get conditionOptions() {
    return [
      {
        label: this.LABELS.MobileDashboard_card_modal_details_filter_logic_and,
        value: "AND"
      },
      {
        label: this.LABELS.MobileDashboard_card_modal_details_filter_logic_or,
        value: "OR"
      },
      {
        label:
          this.LABELS.MobileDashboard_card_modal_details_filter_logic_custom,
        value: "CUSTOM"
      }
    ];
  }
  ...
  @api handleAddConditionClick = () => {
    let newCard = JSON.parse(JSON.stringify(this.tempCard));
    newCard.filter.subFilters.push({
      ...this.EMPTY_SUB_FILTER,
  ...
  @api deleteSubFilter = async (index) => {
    const toDelete = await AlertMessage.open({
      ...
      message:
        this.LABELS.MobileDashboard_settings_delete_sub_filter_warning_body,
  ...
  updateMainSubFilter = (index, newVals) => {
    ...
    newCard.filter.subFilters[index] = {
      ...newCard.filter.subFilters[index],
      ...newVals
    };
    this.tempCard = { ...newCard };
  ...
  async refreshFieldsOptions(obj) {
    ...
    this.fieldsOptions = allFields.sort((f1, f2) => {
  ...
  get mainSubFilters() {
    ...
    subFilters = subFilters.map((s, i) => ({
      ...s,
      displayText: this.getLogicDisplayText(i)
    }));
    return subFilters;
  }
  ...
  getLogicDisplayText(ind) {
    ...
    if (ind === 0) return "";
    return this.conditionLogicText;
  }
  ...
  checkValidation() {
    ...
    if (customLogic)
      inputs.push(this.template.querySelector(".custom-logic-input"));
    ...
    if (
      JSON.stringify(this.subFilters) ===
      JSON.stringify([this.EMPTY_SUB_FILTER])
    ) {
      isValid = false;
