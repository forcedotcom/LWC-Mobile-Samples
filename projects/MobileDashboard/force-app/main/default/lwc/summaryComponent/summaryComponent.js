...
export default class SummaryComponent extends LightningElement {
  ...
  get subContainerClass() {
    return `sub-container ${
      this.layout === "SIDE" ? "sub-container-side" : "sub-container-stack"
    }`;
  }
  ...
  get title() {
    return this.isDemoMode
      ? this.settings?.Title__c
      : this.settings?.node?.Title__c.value;
  }

  get layout() {
    return this.isDemoMode
      ? this.settings?.Layout__c
      : this.settings?.node?.Layout__c.value;
  }
  ...
  get isNonEmptyCard() {
    return this.isDemoMode
      ? !!this.settings.subQueriesData.length
      : !!this.subFilters.length;
  }
