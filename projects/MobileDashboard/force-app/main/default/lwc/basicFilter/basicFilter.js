...
export default class BasicFilter extends LightningElement {
  ...
  get isFieldTypeDate() {
    return (
      this.filterFieldType === "DATE" || this.filterFieldType === "DATETIME"
    );
  }
  ...
  get isFieldTypeRegular() {
    return (
      (!this.isFieldTypeBoolean &&
        !this.isFieldTypePicklist &&
        !this.isFieldTypeDate) ||
  ...
  onFilterFieldChange = (e) => {
    ...
    setTimeout(() => {
      ...
      this.operator = this.isFieldNoOperator
        ? "eq"
        : this.isFieldTypeDate
          ? "today"
          : "eq";
      ...
      this.updateSubFilter(this.index, {
        field: this.field,
        fieldDisplay: this.fieldsOptions?.find(
          (opt) => opt.value === this.field
        )?.label,
        fieldType: this.fieldsOptions?.find((opt) => opt.value === this.field)
          ?.type,
  ...
  get filterOperatorOptions() {
    return [
      ...
      {
        label:
          this.LABELS.MobileDashboard_basic_filter_operator_greater_or_equal,
        value: "gte",
        display: "≥"
      },
      {
    ...
    return [
      {
        ...
        value: "today",
        display:
          this.LABELS.MobileDashboard_settings_card_display_operator_today
      },
      {
      ...
      {
        label: this.LABELS.MobileDashboard_basic_filter_date_type_on,
        value: "eq",
        display: "="
      },
      {
  ...
  getOperatorDisplay() {
    return (
      this.isFieldTypeDate
        ? this.dateFilterOperatorOptions
  ...
  get valueFieldType() {
    if (
      this.SUPPORTED_INPUT_TYPES.includes(this.filterFieldType?.toLowerCase())
    ) {
  ...
  async refreshPicklistOptions(field) {
    ...
    if (!this.filterFieldType) {
      ...
      const allOptions = await getAllPicklistOptions({
        obj: this.objectValue,
        field
      });
      this.picklistOptions = allOptions.sort((f1, f2) => {
  ...
  getUnitDisplay() {
    return this.filterUnitValueOptions?.find((opt) => opt.value === this.unit)
      ?.display;
  }
