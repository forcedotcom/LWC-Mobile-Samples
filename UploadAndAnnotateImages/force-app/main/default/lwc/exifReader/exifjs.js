const EXIF = (function () {
    ...
    function getImageData(img, callback) {
        function handleBinaryFile(binFile) {
            ...
            retImg.exifdata = data || {};
            const iptcdata = findIPTCinJPEG(binFile);
            retImg.iptcdata = iptcdata || {};
            if (EXIF.isXmpEnabled) {
                const xmpdata = findXMPinJPEG(binFile);
                retImg.xmpdata = xmpdata || {};
            }
            if (callback) {
                callback.call(retImg);
            }
        }

        const arrayBuffer = base64ToArrayBuffer(img);
    ...
    function findIPTCinJPEG(file) {
        ...
        const isFieldSegmentStart = function (_dataView, _offset) {
            return (
            ...
            if (xml.children.length > 0) {
                for (let i = 0; i < xml.children.length; i++) {
                    ...
                    for (let idx in attributes) {
                        if(Object.hasOwn(attributes, idx)) {
    ...
    function fraction(numerator, denominator) {
        let _numerator = numerator || 0;
        let _denominator = denominator || 1;
    ...
    _EXIF.getTag = function (img, tag) {
        if (!imageHasData(img)) return null;
