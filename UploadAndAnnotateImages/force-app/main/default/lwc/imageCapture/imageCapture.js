...
import { log, IMAGE_EXT, isNullOrEmpty, waitMillis, ToastTypes } from "c/utilsImageCapture";
...
export default class ImageCapture extends LightningElement {
    ...
    get toastMessage() {
        switch (this.toastType) {
            case ToastTypes.Success: {
                const imageString = this.numPhotosToUpload > 1 ? "images were" : "image was";
    ...
    async handleImagesSelected(event) {
        ...
        log(`Reading ${compressionEnabled ? "and compressing " : ""}${numFiles} images`);
    ...
    readFile(file) {
        return new Promise((resolve, reject) => {
            ...
            reader.onerror = () => { 
                reject(`There was an error reading file: '${file.name}', error: ${reader.error}`); }
    ...
    readMetadata(file) {
        return new Promise((resolve) => {
            ...
            const ext = fullFileName.slice((Math.max(0, fullFileName.lastIndexOf(".")) || Infinity) + 1);
            const fileNameWithoutExt = fullFileName.substring(0, fullFileName.length - ext.length - (ext ? 1 : 0));
    ...
    async handleUploadRequested() {
        ...
        try {
            ...
            if (this.numPhotosToUpload > 1 && this.numSuccessfullyUploadedPhotos > 0) {
    ...
    async getIdsAndCreateCdl(contentVersion, recordId, uniqueCvId) {
        ...
        var contentVersionId = await this.getContentVersionId(contentVersion, uniqueCvId);
    ...
    async waitForDraftToUpload(uniqueCvId) {
        ...
        const SEC_TO_WAIT = 1;
        log(`Querying the org up to ${MAX_RETRIES} times with ${SEC_TO_WAIT} sec between each call, waiting for the draft to get there...`);

        for (let i = 0; i < MAX_RETRIES; i++) {
            let receivedId = "";
            await getContentVersionId(
                { uniqueCvId: uniqueCvId }
            ).then((res) => {
    ...
    async getContentDocumentId(contentVersionId) {
        ...
        if (isNullOrEmpty(contentDocumentId)) {
            throw new Error(`Failed to retrieve ContentDocument ID for ContentVersion ID ${contentVersionId}!`);
        }
