...
export default class MobileMapLayersMain extends NavigationMixin(LightningElement) {
    ...
    currentMarker;
    currentMarkerInd = 0;
    currentObjectFilter = { value: '', label: '', plural: '', iconUrl: '', color: '' };
    currentFieldFilter = { isActive: false, field: { value: '', label: '', type: '', input: '' } };
    init = false;
        ...
        if (!this.init) {
            ...
            window.visualViewport?.addEventListener('resize', e => {
                if (window.visualViewport.height === window.innerHeight) 
                    this.template.querySelector('.find-nearby-main-container').style.top = `${window.visualViewport.pageTop.toString()}px`;
            });
        }
    }
    ...
    async addLocations() {
        try {
            ...
            if (resourceLocation?.length && resourceLocation[0] && resourceLocation[1]) {
                this.resourceLocation = { lat: resourceLocation[0], lng: resourceLocation[1] };
            }
            ...
            this.CONFIG.mapObjects.forEach(async (o, ind) => {
                ...
                if (ind === this.CONFIG.mapObjects.length - 1) this.CONFIG.mapObjects = [ ...this.CONFIG.mapObjects ];
    ...
    async updateFilteredMarkers() {
        if (this.currentFieldFilter.isActive) {
            ...
            try {
                ...
                this.filteredMarkers = this.allMarkers.filter(m => ids.map(o => o.Id).includes(m.value.id));
            }

        } else {
            this.filteredMarkers = this.allMarkers.filter(m => m.value.type === this.currentObjectFilter.value);
        }
