...
export default class MobileAppointmentBookingRescheduleAppointment extends LightningElement {
    ...
    handleCloseModal(){ // close the modal without scheduling
        const customEvent = new CustomEvent('closeslotmodal');
        this.dispatchEvent(customEvent);
    }

    handleConfirmBtnClose(){ // close the modal window after scheduling success of fail
        document.body.style.overflow = 'auto';
        const customEvent = new CustomEvent('scheduleclosewindow');
        this.dispatchEvent(customEvent);
    }

    handleConfirm(){ // press on Book Appointment button to schedule the SA
        this.showSpinner = true;
        const customEvent = new CustomEvent('scheduleconfirmed');
        this.dispatchEvent(customEvent);
    }

    @api handleSchedulingResponse(response){ // calls at the end of scheduling process
