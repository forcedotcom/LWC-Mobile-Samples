...
export default class MobileAppointmentBookingSlotsContainer extends LightningElement {
    ...
    MONTHNAME = [ this.LABELS.FollowUpAppointments_MonthName_January,
    ...
    DAYNAME = [ this.LABELS.FollowUpAppointments_WeekDayLong_Sunday,
                    this.LABELS.FollowUpAppointments_WeekDayLong_Monday,
                    this.LABELS.FollowUpAppointments_WeekDayLong_Tuesday,
                    this.LABELS.FollowUpAppointments_WeekDayLong_Wednesday,
                    this.LABELS.FollowUpAppointments_WeekDayLong_Thursday,
                    this.LABELS.FollowUpAppointments_WeekDayLong_Friday,
                    this.LABELS.FollowUpAppointments_WeekDayLong_Saturday ];
            ...
            if((date <= maxDate)) {
                ...
                for(let k=0; k < timeSlotArray.length; k++) {
                    ...
                    if(date === timeSlotArray[k].dateInMiliSec) {
                        ...
                        timeDateArray.isRecomended = timeSlotArray[k].grade > this.recommendedScore ? true : false;
    ...
    formatTitle(value) {
        ...
        if(date.setHours(0,0,0,0) === new Date().setHours(0,0,0,0)) {
            title = title + " , "+this.LABELS.FollowUpAppointments_today_text;
        }
        return title;
    }
    ...
    sortTimeSlotAccordingToWeekSelected(dateWiseSlotArray) {
        ...
        for(let i=0; i < dateWiseSlotArray.length; i++) {
            ...
            }  // removed empty else as part of lint issue
        }
    ...
    addNonAvailableSlotsToList(newSortedArray) {
        ...
        loopDate = new Date(this.firstDayOfTheWeek);
        currDate = this.getDateWithoutTime(Date.parse(new Date()));
        ...
        while(loopDate <= this.lastDayOfTheWeek) {
            ...
            // Todo : newDate is declared twice. Check correct behaviour
    ...
    getFirstDayOfWeek(date, index) {
        ...
        let newDate = new Date(d.setDate(d.getDate() - this.showNoOfDaysBeforeAfterWeek)).setHours(0,0,0,0);
        return newDate;
    };
    ...
    executeScroll(selectedDate) {
        ...
        if(elementToShow) {
            try {
                ...
                let elementToShowLocation = elementToShow.getBoundingClientRect(); //gets the current user view
                let offset = elementToShowLocation.top -235; //gets the difference between user view and element view minus the calendar height
