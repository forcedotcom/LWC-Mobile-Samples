...
export default class followUpAppointmentMain extends LightningElement {
    ...
    connectedCallback() {
        ...
        this.lockScrolling();
        getSettingsObject({recordId: this.recordId}).then((settings) => {
            if(settings.error || settings.error2) {
                ...
                if (this.settings.fieldsToShowPage1 === 'null' && this.settings.fieldsToShowPage2 === 'null' && this.settings.fieldsToShowPage3 === 'null') {
                    this.allowScrolling();
                    this.hideNoSetupError = false;
                    this.hideHeader = true;
                    this.hideNextButton = true;
                    this.hideBackButton = true;
                    console.log("*** We couldn’t launch this action. Ask your admin for help. ***")
                } else {
                            ...
                            this.settings['fieldsToShowPage'+index].split(';').forEach(field => {
                                if (field.split(',')[0] !== '') {
                                    ...
                                    switch (field.split(',')[0]) {
                                        case 'WorkOrder' :
                                            this['fieldsToRecordFormPage'+index].push({objectApiName: field.split(',')[0], fieldApiName: field.split(',')[1], recordId: this.recordId});
                                            break;
                                        case 'WorkOrderLineItem' :
                                            this['fieldsToRecordFormPage'+index].push({objectApiName: field.split(',')[0], fieldApiName: field.split(',')[1], recordId: this.recordId});
                                            break;
                                        case 'ServiceAppointment' :
                                            this['fieldsToRecordFormPage'+index].push({objectApiName: field.split(',')[0], fieldApiName: field.split(',')[1], recordId: settings.serviceAppointmentId});
                                            break;
                                        default : {
                                            break;
                                        }
    ...
    callCreateRecordClass(arrivalWindowStartTime, arrivalWindowEndTime) {
        ...
        querySelector.forEach(field => {
            var index = this.listOfFields.findIndex(i => i.includes(field.parentElement.objectApiName+','+field.fieldName+','));
            this.listOfFields[index] = this.listOfFields[index] + ',' + field.value;
            console.log('-- ', this.listOfFields[index]);
        });

        createRecord({recordToCreate: this.settings.recordToCreate,
...
// ---------------------------------------------------------------------------------------------------------------
    ...
    handleFieldsAreLoaded() {
        ...
        let numberOfFields = this.fieldsToRecordFormPage1.length + this.fieldsToRecordFormPage2.length + this.fieldsToRecordFormPage3.length;
