...
export default class MobileWorkFlowPage extends LightningElement {
        ...
        for (let i = 0; i < this.selected.length; i++) {
            switch (this.selectedObject) {
                case "ServiceAppointment":
                    strField = strField + this.selectedObject + "," + this.selected[i] + "," + this.serviceAppointmentFields.get(this.selected[i]) + ";";
                    break;
                case "WorkOrder":
                    strField = strField + this.selectedObject + "," + this.selected[i] + "," + this.workOrderFields.get(this.selected[i]) + ";";
                    break;
                case "WorkOrderLineItem":
                    strField = strField + this.selectedObject + "," + this.selected[i] + "," + this.workOrderLineItemFields.get(this.selected[i]) + ";";
                    break;
                default : 
                    break;
            }
    ...
    handleSaveFieldDataApexCall(arr) {
        ...
        .then((data) => {
            if (data.error) {
                ...
                if(this.dialgBoxTitle === this.LABELS.FollowUpAppointments_new_step_text_label) {
                    this.showToastMessages(this.LABELS.FollowUpAppointments_settings_createScreenToastMessage.replace('{0}', arr[1]), true);
                } else {
        ...
        .catch((error) => {
            ...
            this.dataLoaded = true;
            if(this.dialgBoxTitle === this.LABELS.FollowUpAppointments_new_step_text_label) {
                this.showToastMessages( this.LABELS.FollowUpAppointments_workflow_addStepFailureMessage, false );
            } else {
    ...
    callAPEX() {
        ...
        .then((data) => {
            if (data.error) {
                ...
                if ( data.Screen_Title_1__c && data.Screen_Title_1__c.length > 0 && data.Screen_Title_1__c !== "null" ) {
                ...
                if ( data.Screen_Title_2__c && data.Screen_Title_2__c.length > 0 && data.Screen_Title_2__c !== "null" ) {
                ...
                if ( data.Screen_Title_3__c && data.Screen_Title_3__c.length > 0 && data.Screen_Title_3__c !== "null" ) {
    ...
    filter(event) {
        let filter = event ? new RegExp(this.template.querySelector("[data-name='searchfield']").value,"ig")
        : {
            test: function () {
                return true;
            }
    ...
    enableDisableSaveButton() {
        if (this.currentScreenTitle && this.currentScreenTitle.length > 0 && this.selected && this.selected.length > 0 ) {
            this.isSaveButtonDisabled = false;
        } else {
            this.isSaveButtonDisabled = true;
        }
        ...
        deleteScreen({ screenNo: this.deletePageNoCalled })
            .then((data) => {
                if (data.error) {
                    ...
                    this.showToastMessages(this.LABELS.FollowUpAppointments_settings_deleteScreenToastMessage.replace('{0}', screenTitleTemp), true);
                    this.currentScreenDisplayed = this.currentScreenDisplayed - 1;

                    this.currentScreenDisplayed === this.screenMaxLimitConst ? (this.disableAddStepBtn = true) : (this.disableAddStepBtn = false);
            ...
            .catch((error) => {
                ...
                this.dataLoaded = true;
                this.showToastMessages(this.LABELS.FollowUpAppointments_settingPage_error_delete_screen , false);
            });
        } else {
            ...
            if (toScreenNo) {
                ...
                .then((data) => {
                    if (data.error) {
                        ...
                        this.callAPEX();
                        this.showToastMessages(this.LABELS.FollowUpAppointments_settings_duplicateScreenToastMessage.replace('{0}', screenTitleTemp), true);
                    }
                })
                .catch((error) => {
                    console.log("Error while duplicating the screen : " + error.message);
                    this.dataLoaded = true;
                    this.showToastMessages(this.LABELS.FollowUpAppointments_settingPage_error_duplicate_screen, false);
                });
            }
        }
