...
// Commented the assignmentMethod as its not used : lint fixes
...
export default class MobileAppointmentBookingSchedulingContainer extends LightningElement {
    ...
    @api newRecordNumber;
    @api contactName;
    @api serviceTerritoryTimeZone;

    show_confirmBtnLayout = false;
    @api recommendedScore;
    @api allAppointmentsTitle = this.LABELS.FollowUpAppointments_all_available_appointments;
    @api recommendedAppointmentsTitle = this.LABELS.FollowUpAppointments_recommended_appointments;
    ...
    @api get serviceterritorytimezone() {
    ...
    onDateSelected(event) {
        ...
        this.template.querySelector('c-mobile-appointment-booking-slots-container').onPositionUpdated(top);    // removed the assignment because of lint
    }

    onWeekChangeEvent(event) {
        this.selectedDate = event.detail.date;
        console.log("On week change called");
        this.template.querySelector('c-mobile-appointment-booking-slots-container').onWeekUpdated(this.selectedDate);    //removed the assignment because of lint
    ...
    convertDateUTCtoLocal(date) {
        ...
        return '';    // removed else part because of lint;
    ...
    @api handleSchedulingResponse(response){
        this.template.querySelector('c-mobile-appointment-booking-reschedule-appointment').handleSchedulingResponse(response);
    }
