...
export default class followUpAppointmentSettingsContainer extends LightningElement {
    ...
    callAPEX() {
        ...
        this.objectList = [];
        getSchedulingPolicyList().then((data) => {
            if (data.error) {
        ...
        getOperatingHoursList().then((data) => {
            if (data.error) {
                throw new Error(data.error);
            } else {
                for (let i = 0; i < data.length; i++) {
        ...
        this.slotDisplayOptions = [...this.slotDisplayOptions, { value: this.arrival_window, label: this.arrival_window }];
        this.slotDisplayOptions = [...this.slotDisplayOptions, { value: this.exact_appointment_time, label: this.exact_appointment_time }];
        ...
        getConfigurationData()
            .then((settings) => {
                if (settings.error) {
                    ...
                    this.schedulingHorizonUnits = this.savedSchedulingHorizonUnits = settings.schedulingHorizonValue;
                    ...
                    this.technicianAssigmentSelected = this.savedTechnicianAssignment = settings.Technician_Assigment__c;
                    ...
                    myArray[0] === "workOrderLineItem" ? (this.showWOLISAoption = true) : (this.showWOLISAoption = false);

                    if (myArray.length > 1 && myArray[1].length > 0) {
                        myArray[0] === "workOrder" ? (this.isSAWOselected = true) : (this.isSAWOLIselected = true);
                    }
                    this.validateSaveButton();
                    this.dataLoaded = true;
                }
    ...
    get assignmentPermissionOptions() {
        return [
            {label:this.LABELS.FollowUpAppointments_MobileWorkerAsignTo_themselves_or_anyWorker,value: "1"},
            {label: this.LABELS.FollowUpAppointments_MobileWorkerAsignTo_any_worker,value: "2"},
            {label: this.LABELS.FollowUpAppointments_MobileWorkerAsignTo_themselves,value: "3"}];
    }
    ...
    onAssignementPermissionChange(event) {
        ...
        this.savedTechnicianAssignment !== this.selectedAssigmentPermission ? this.hasUnsavedChanges = true : this.hasUnsavedChanges = false;
    ...
    onMobileUserObjectSelected(event) {
        ...
        this.savedObjectCreation !== this.objectCreationAllowed ? this.hasUnsavedChanges = true : this.hasUnsavedChanges = false;
        this.validateSaveButton();
    }
    ...
    @api handleSaveEvent() {
        ...
        this.objectCreationAllowed === "ServiceAppointment" ? (objectAllowedToBeCreated = objectAllowedToBeCreated + "ServiceAppointment") : objectAllowedToBeCreated;
        ...
        .then((data) => {
            if (data.error) {
                ...
                this.showToastMessages( this.LABELS.FollowUpAppointments_settingPage_save_message, this.successVariant);
        ...
        .catch((error) => {
            console.log("Error while getting scheduling policy : " + error);
            this.showToastMessages( this.LABELS.FollowUpAppointments_settingPage_error_message + " : " + error.message, this.errorVariant);
        });
    }
    ...
    handleTabChangeEvent() {
        if(this.hasUnsavedChanges) {
            this.showToastMessages( this.LABELS.FollowUpAppointments_settingPage_warning_message, this.warningVariant);
        }
        this.reloadChildComponent ? this.reloadChildComponent= false : this.reloadChildComponent= true;
    ...
    handleSchedulingPolicyChange(event) {
        ...
        this.savedSP !== this.schedulingPolicySelected ? this.hasUnsavedChanges = true : this.hasUnsavedChanges = false;
        this.validateSaveButton();
    }

    handleOperatingHoursChange(event) {
        this.operatingHoursSelected = event.detail.value;
        this.savedOH !== this.operatingHoursSelected ? this.hasUnsavedChanges = true : this.hasUnsavedChanges = false;
        this.validateSaveButton();
    }

    handleAppointmentSlotSelectionChange(event) {
        this.appointmentSlotOptionSelected = event.detail.value;
        this.savedAppointmentSlotSelected !== this.appointmentSlotOptionSelected ? this.hasUnsavedChanges = true : this.hasUnsavedChanges = false;
        this.validateSaveButton();
    }
    ...
    onSchedulingHorizonValueChange(event) {
        ...
        this.savedSchedulingHorizonUnits !== this.schedulingHorizonUnits ? this.hasUnsavedChanges = true : this.hasUnsavedChanges = false;
        this.validateSaveButton();
    }
    ...
    showToastMessages(message, variant) {
        this.template.querySelector("c-follow-up-appointment-custom-toast-notifications").showToast(variant, message);
    }
    ...
    validateSaveButton() {
        if (this.schedulingPolicySelected && this.operatingHoursSelected && this.appointmentSlotOptionSelected && this.objectCreationAllowed && this.technicianAssigmentSelected && this.schedulingHorizonUnits ) {
            if(this.schedulingPolicySelected === this.savedSP &&  this.operatingHoursSelected === this.savedOH && this.appointmentSlotOptionSelected === this.savedAppointmentSlotSelected 
                && this.objectCreationAllowed === this.savedObjectCreation && this.technicianAssigmentSelected === this.savedTechnicianAssignment && 
