...
describe('c-follow-up-appointment-settings-container', () => {
  ...
  it('should show correct info', () => {
    const fieldServiceLabel = element.shadowRoot.querySelector('.fieldservicelabel');
    expect(fieldServiceLabel.textContent).toBe('c.FollowUpAppointments_field_service_title');

    const mainTitleElement = element.shadowRoot.querySelector('.followupmaintitle');
    expect(mainTitleElement.textContent).toBe('c.FollowUpAppointments_setting_page_title');

    const pageTitle = element.shadowRoot.querySelector('.pageTitle');
    expect(pageTitle.textContent).toBe('c.FollowUpAppointments_setting_sub_title');

    const pageDescription = element.shadowRoot.querySelector('.pageDescription');
    expect(pageDescription.textContent).toBe('c.FollowUpAppointments_setting_description_text');
  });
